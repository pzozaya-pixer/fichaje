...
async function verifyTurnstileToken(token: string | null): Promise<boolean> {
  ...
  // Si el token es un token de prueba (de desarrollo en localhost), usamos la clave secreta de sandbox de Cloudflare
  if (token.startsWith('1x00000000') || token === 'XXXX.dummy.token.XXXX') {
    console.log('Turnstile: Detectado token de prueba local. Usando clave secreta de sandbox.');
